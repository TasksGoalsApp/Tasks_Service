...
class JwtSecurityTests {
    ...
    @Test void missingAndMalformedTokensAreUnauthorized() throws Exception {
        mvc.perform(get("/tasks/user")).andExpect(status().isUnauthorized());
        mvc.perform(get("/tasks/user").header("Authorization", "Bearer nonsense")).andExpect(status().isUnauthorized());
    }
    @Test void expiredTokenIsUnauthorized() throws Exception {
        ...
        mvc.perform(get("/tasks/user").header("Authorization", "Bearer " + token)).andExpect(status().isUnauthorized());
    }
    ...
    @Test void wrongSignatureIsUnauthorized() throws Exception {
        ...
        mvc.perform(get("/tasks/user").header("Authorization", "Bearer " + tampered)).andExpect(status().isUnauthorized());
    }
    @Test void authenticatedWrongRoleIsForbidden() throws Exception {
        mvc.perform(get("/tasks/user").header("Authorization", "Bearer " + TestTokens.token(1,"VIEWER")))
                .andExpect(status().isForbidden());
    }
    @Test void corsAllowsOnlyConfiguredOrigin() throws Exception {
        mvc.perform(options("/tasks/user").header("Origin","http://localhost:5173")
                .header("Access-Control-Request-Method","GET").header("Access-Control-Request-Headers","authorization"))
                .andExpect(status().isOk()).andExpect(header().string("Access-Control-Allow-Origin","http://localhost:5173"));
        mvc.perform(options("/tasks/user").header("Origin","https://untrusted.example")
